...
// const designSystems = Array.from(libraries)
// const designSystems = arrayToObject(libraries, "id")
  ...
  render() {
    return (
      <Router>
        <Route path="/*">
          <aside id="tw">
            <nav className="bg-gray-800 text-white min-h-full p-4 flex justify-between items-center">
              <div className="flex gap-3 items-center">
                <h1 className="m-0 text-current font-bold text-xl">Design System Sandbox</h1>
                <Route path="/:id">
                  <span>•</span>
                  <h2 className="m-0 text-current font-bold text-xl"><Name /></h2>
                </Route>
              </div>
            ...
            {
              libraries.map(({ route, id, name, code }) => {
        ...
        {/* <Switch>
            {
              libraries.map(({ scope, id, code, route }) => {
